...
static const char *TAG = "MQTTLinux";

/* Root cert for thingsboard, found in cert.c */
extern const char *srv_crt_rsa;
extern const char *cli_crt_rsa;
extern const char *cli_key_rsa;
...
int NetworkConnect(Network* n, char* addr, int port) {
    ...
    mbedtls_x509_crt_init(&n->srvcert);
    mbedtls_x509_crt_init(&n->cachain);
    ...
    mbedtls_net_init(&n->server_fd);
	  ...
	  ESP_LOGD(TAG, "Connecting to %s:%s...", addr, portbuf);
    ...
    ret = mbedtls_x509_crt_parse(&n->srvcert, (uint8_t*)srv_crt_rsa, strlen(srv_crt_rsa)+1);
    if(ret < 0)
    {
   ...
   ret = mbedtls_x509_crt_parse(&n->cachain, (uint8_t*)cli_crt_rsa, strlen(cli_crt_rsa)+1);
    if(ret < 0)
    {
        ESP_LOGE(TAG, "mbedtls_x509_crt_parse returned -0x%x\n\n", -ret);
        abort();
   ...
   ret = mbedtls_pk_parse_key(&n->pkey, (uint8_t*)cli_key_rsa, strlen(cli_key_rsa)+1, NULL, 0);
    if(ret < 0)
    {
    ...
    */
    ...
    mbedtls_ssl_conf_own_cert(&n->conf, &n->cachain, &n->pkey);
    mbedtls_ssl_conf_ca_chain(&n->conf, &n->srvcert, NULL);
...
void NetworkDisconnect(Network* n) {
	...
	*/
	...
	mbedtls_x509_crt_free(&n->srvcert);
  ESP_LOGD(TAG, "mbedtls_x509_crt_free");
	mbedtls_x509_crt_free(&n->cachain);
